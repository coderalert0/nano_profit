...
module Pricing
  class SyncService
    ...
    def perform
      ...
      # Pre-load all global rates to avoid N individual queries
      existing_rates = VendorRate.where(organization_id: nil)
        .index_by { |r| [r.vendor_name, r.ai_model_name] }
      ...
      models.each do |key, entry|
        ...
        existing = existing_rates[[vendor, model]]
      ...
      ActiveRecord::Base.transaction do
        ...
        if stale_ids.any?
          counts[:deactivated] = VendorRate.where(id: stale_ids).update_all(active: false)
        end
      end
    ...
    def normalize_rate(entry, direction)
      ...
      if entry[token_key].present?
        ...
        provider = match_provider(entry["litellm_provider"])
        cpt = CHARS_PER_TOKEN.fetch(provider, DEFAULT_CHARS_PER_TOKEN)
        entry[char_key].to_d * cpt * 100_000  # dollars/char → cents/1K tokens
      end
    end
